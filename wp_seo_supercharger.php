...
if ( !class_exists( "WPSEOSupercharger" ) ) {
	class WPSEOSupercharger {
		...
		var $setting_page = '';
		var $options = '';
		var $option_name = 'WPSEOSupercharger';
		var $error ='';
		var $notify=false;
		...
		function get_current_url() {
			...
			$cur = $schema.$_SERVER["SERVER_NAME"].$_SERVER["REQUEST_URI"];
			return $cur;
		}
		...
		function add_menu_pages() {
			...
			$m0=add_submenu_page( 'wp_seo_supercharger_settings_page', 'Settings Page', 'Settings Page', 'manage_options', 'wp_seo_supercharger_settings_page',  array( &$this, 'settings_page' ) );
...
<p><img style="width:128px;float:left;margin-right:5px;" src="[rimage_url]" />[rpost_content:200]...<a href="[rpost_link]">Continue reading</a></p>
<hr />',
		...
		function settings_page() {
			...
			if ( wp_verify_nonce( $_POST['wp_seo_supercharger_settings_page'], 'wp_seo_supercharger_settings_page' ) ) {
				...
				$this->redirect_to_current_page( $tab );
			}
			extract( $o );
		...
		function redirect_to_current_page( $tab='general-settings' ) {

			$this->redirect_to_page( admin_url( 'admin.php?page='.$_REQUEST['page'].'&success#'.$tab ) );
		}
